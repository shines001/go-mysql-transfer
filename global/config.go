...
type Config struct {
	...
	// ------------------- MYSQL -------------------
	MysqlAddr string `yaml:"mysql_addrs"`
...
/*
 * 功能：检查必填项,初始化默认配置
 * target, addr, user , pass ,charset,slave_id,flavor 必须设置，不能为空
 */
func checkConfig(c *Config) error {
	...
	if c.FlushBulkInterval == 0 {
		c.FlushBulkInterval = _flushBulkInterval //200
	}

	if c.BulkSize == 0 {
		c.BulkSize = _flushBulkSize //100  批量数量
	}
...
func checkMysqlConfig(c *Config) error {
	if len(c.MysqlAddr) == 0 || len(c.MysqlUser) == 0 || len(c.MysqlPass) == 0 {
		return errors.Errorf("empty mysql_addr,mysql_user or mysql_pass not allowed")
	}
...
/*
 * 返回目标服务以及地址，例如： redis(127.0.0.1:6379)
 */
...
/*
 *返回目标的标准名称，例如： Redis RocketMQ  MongoDB
 */
